	...
	class TcpSocket : IPSocket<ByteBuffer, IByteStream>, ITcpSocket, ITcpServerSocket
	{
				...
				Context.Enqueue (delegate {
					if (!parent.disposed) {
						...
						if (error != System.Net.Sockets.SocketError.Success) {
							RaiseError (new Waser.IO.SocketException ("Read failure", Errors.ErrorToSocketError (error)));
						} else if (len == 0) {
							RaiseEndOfStream ();
							Close ();
						} else {
				...
				Context.Enqueue (delegate {
					if (!parent.disposed) {
						...
						if (err != System.Net.Sockets.SocketError.Success) {
							RaiseError (new Waser.IO.SocketException ("Write failure", Errors.ErrorToSocketError (err)));
						} else {
							HandleWrite ();
						}
					}
				...
				Context.Enqueue (delegate {
					if (!disposed) {
						try {
							...
							error (new Waser.IO.SocketException ("Connect failure", Errors.ErrorToSocketError (e.SocketErrorCode)));
						}
					}
				});
			}, null);
		...
		public void Listen (int backlog, Action<ITcpSocket> callback)
		{
			try {
				...
				throw new Waser.IO.SocketException ("Listen failure", Errors.ErrorToSocketError (e.SocketErrorCode));
			}
		}
