...
namespace Waser.IO
{
	static class Errors
	{
		...
		public static IOException SocketStreamFailure (string text, int errno)
		{
			...
			if (err != null) {
				return new IOException (text, new Waser.IO.SocketException (err.Value));
			} else {
				return new IOException (text, new InvalidOperationException (ErrorToString (errno)));
			}
		}
		
		public static Waser.IO.SocketException SocketFailure (string text, int errno)
		{
			var err = ErrorToSocketError (errno);
			return new Waser.IO.SocketException (text, err ?? SocketError.Failure);
		}
	}
}
